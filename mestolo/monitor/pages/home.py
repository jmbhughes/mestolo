...
from mestolo.core import get_database_session, RecipeRunDB
...
def update_cards(n):
    engine, session = get_database_session()
    query = "SELECT count(*) FROM recipe_runs WHERE end_time IS NULL;"
    df = pd.read_sql_query(query, session.connection())
    cards = [dbc.Col(dbc.Card(dbc.CardBody(
        [html.H5("Running", className="card-title"),
         html.P(df['count(*)'].values[0], className="card-text")]
    )))]
